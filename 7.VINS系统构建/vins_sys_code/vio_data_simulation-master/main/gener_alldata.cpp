...
std::vector < std::pair< Eigen::Vector4d, Eigen::Vector4d > > CreatePointsLines(std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d> >& points)
{
...
//    std::cout << Qwb.coeffs().transpose() <<"\n"<<Qwb.toRotationMatrix() << std::endl;
    ...
    save_cam_Pose("cam_pose.txt",camdata);
    save_Pose_asTUM("cam_pose_tum.txt",camdata);
    
    // points obs in image
    std::string  filename="image_filename.txt";
    std::ofstream save_image_filename;
    save_image_filename.open(filename.c_str());
    ...
    for(int n = 0; n < camdata.size(); ++n)
    {
        ...
        save_image_filename<<data.timestamp << "  " <<"keyframe/all_points_"<<n<<".txt"<<std::endl;
    }
     save_image_filename.close();
