...
IMU::IMU(Param p): param_(p)
{
 ...
 //http://c.biancheng.net/view/643.html  STL  下面
...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 0; i < imudata.size()-1; ++i) {
        ...
        // //中值积分delta_q = [1 , 1/2 * thetax , 1/2 * theta_y, 1/2 * theta_z]
        // Eigen::Quaterniond dq;

        // MotionData imupose_start = imudata[i];
        // MotionData imupose_end   = imudata[i+1];

        // Eigen::Vector3d dtheta_half =0.5*( imupose_start.imu_gyro +imupose_end.imu_gyro )* dt /2.0;
        // dq.w() = 1;
        // dq.x() = dtheta_half.x();
        // dq.y() = dtheta_half.y();
        // dq.z() = dtheta_half.z();

        // Eigen::Vector3d acc_w = (Qwb * (imupose_start.imu_acc) +gw + Qwb * dq * (imupose_end.imu_acc)+gw)/2.0;  
        // std::cout<<acc_w<<std::endl;
